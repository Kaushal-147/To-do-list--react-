...
function App() 
{
    ...
    { 
        ...
        else
        {
            ...
            console.log(todos , "before save");
            saveTodo();
            console.log(todos , "after save");
    ...
    { 
        ...
        newtodos[index].todo = document.getElementById("Input").value;
        setTodos(newtodos);
        saveTodo();
        setTodo("");
        setAddsave("Add"); 
    ...
    <div className="bg-gradient-to-r from-[#565657] to-[#092F41] min-h-screen flex flex-col  items-center ">
        ...
        <div className="input-bar min-w-full flex justify-center items-center h-12 mb-6">
           {/*--------------------------------------Main  Input bar -------------------------------------- */}
          <input type="text" placeholder="Add a task" id="Input" onChange={handleInputchange} onKeyDown={(e) => {if(e.key === "Enter") handleAdd(); }} ref={inputRef} className="input-box rounded-xl h-3/4 w-1/3 rounded-tr-[0] rounded-br-[0] p-4" value={todo}  />
           {/*-------------------------------------------------------------------------------------------- */}
          ...
          {todos.map((item) => 
          {
           ...
           <div key={item.id} className={item.itemclass} >
               <div className="check-box size-8 flex justify-center items-center">
                   <input type='checkbox' name={item.id} onChange={handleCheck} className="rounded bg-gray-800 size-[1.05rem]" value={item.isCompleted} >
                   </input>
               </div>
               <div className={item.isCompleted?"line-through task-name text-[#999192]":"task-name text-white "}>
                 {item.todo}
               </div>
